...
const createDeviceScene = () => {
  ...
  const animate = (time = 0) => {
    ...
    if (topHero > 0.08) {
      const heroScroll = smoothstep(0.02, 0.82, viewportProgress);
      targetX = mix(mobile ? 0.28 : 0.42, mobile ? 1.8 : 3.8, heroScroll);
      targetY = mix(0.02, 0.26, heroScroll) + Math.sin(time * 0.001) * 0.035;
      targetRotY = mix(Math.PI * 0.92, Math.PI * 1.55, heroScroll) + pointer.nx * 0.08 + Math.sin(time * 0.00055) * 0.06;
      targetRotX = mix(-0.08, -0.2, heroScroll) + pointer.ny * 0.05;
      targetRotZ = mix(-0.02, 0.1, heroScroll) + Math.sin(time * 0.0004) * 0.02;
    } else if (inManifesto) {
